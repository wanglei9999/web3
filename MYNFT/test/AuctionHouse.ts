...
describe("AuctionHouse", function () {
  ...
  beforeEach(async function () {
    ...
    await myNFT.safeMint(seller.address, "https://example.com/nft/1");
    await myNFT.connect(seller).setApprovalForAll(auctionHouse.address, true);

    await myERC20.mint(bidder1.address, ethers.utils.parseUnits("1000", 18));
    await myERC20.mint(bidder2.address, ethers.utils.parseUnits("1000", 18));
  });
  ...
  describe("bidAuction", function () {
    ...
    it("should refund previous bidder", async function () {
      ...
      expect(diff < ethers.utils.parseEther("0.1")).to.be.true;
    });
  });
  ...
  describe("UUPS Upgrade", function () {
    it("should upgrade contract", async function () {
      ...
      expect(Number(await upgraded.getVersion())).to.equal(2);
    });
  });
});
